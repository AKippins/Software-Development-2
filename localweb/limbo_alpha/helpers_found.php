...
function show_records($dbc) {
	...
	$query = 'SELECT item_id, object, description, room, finder, date_found, location_id, status FROM inventory ORDER BY item_id ASC' ;
...
function show_link_records($dbc) {
  # Create a query to get the name and price sorted by price
  $query = 'SELECT item_id, object, description, room, finder, date_found, location_id, status FROM inventory WHERE status="lost" ORDER BY item_id ASC' ;
  ...
  if( $results )
  {
      # But...wait until we know the query succeed before
      # rendering the table start.
      echo '<H2>These Are Some Items That Have Been Recently Lost.....</H2>' ;
      echo '<H3>Are Any Of These What You\'ve Found???</H3>' ;
      echo '<TABLE border=1>';
      echo '<TR>';
      echo '<TH align=right>Item ID</TH>';
      echo '<TH>Object</TH>';
      ...
      while ( $row = mysqli_fetch_array( $results , MYSQLI_ASSOC ) )
      {
        $alink = '<A HREF=inventory_found.php?item_id=' . $row['item_id'] . '>' . $row['item_id'] . '</A>' ;
        echo '<TR>' ;
